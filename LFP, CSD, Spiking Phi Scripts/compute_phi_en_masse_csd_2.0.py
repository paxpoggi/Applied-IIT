...
def scatter_plot_phi_values(all_iterations_phi_values, probe_name, visual_area):
    ...
    ax.set_title(f'CSD Phi-values for Different Presentations in {visual_area} for {probe_name}')

    # Set legend with smaller font size
    legend = ax.legend(prop={'size': 8})

    # save figure to folder
    folder_name = '/Users/poggi/Documents/Maier Lab/Figures (Pax)/CSD Scatter Plots'
    file_name = f'phi_values_rand_csd_{visual_area} for {probe_name}'
...
def compute_phi_csd_rand_int(presentation_df, csd_df, interval, channels, sampling_rate):
    ...
    for column in channels:
        values = csd_df[column][start_index:end_index + 1]
        if len(values) != len(binary_df):
            print(f"Size mismatch: binary_df has {len(binary_df)} rows, values from csd_df has {len(values)} rows")
        # Iterate over the rows (data points) within the desired time range
        for i, value in enumerate(csd_df[column][start_index:end_index + 1]):
...
def calculate_csd(lfp_df, channels_df, desired_ids, desired_indices):
    """
    Calculate the Current Source Density (CSD) from Local Field Potential (LFP) data.

    :param lfp_df: DataFrame of LFP data
    :param channels_df: DataFrame containing channel information
    :param desired_ids: List of desired electrode IDs
    :param desired_indices: List of indices in the LFP DataFrame corresponding to the desired IDs
    :return: DataFrame containing the CSD data
    """
    # Extracting vertical and horizontal positions from channels_df
    vertical_positions = np.array(channels_df['probe_vertical_position'])
    horizontal_positions = np.array(channels_df['probe_horizontal_position'])

    # Pairing vertical and horizontal positions with the probe IDs
    positions = np.array([vertical_positions, horizontal_positions]).T
    channel_positions = dict(zip(channels_df['id'], positions))

    probe_ids_and_lfp_and_positions = []

    # Iterating over each pair of desired_id and its corresponding index
    for desired_id, index in zip(desired_ids, desired_indices):
        if desired_id in channel_positions:
            lfp_data_for_probe = lfp_df[index].values
            probe_ids_and_lfp_and_positions.append((desired_id, lfp_data_for_probe, channel_positions[desired_id]))
        else:
            print(f'Probe ID {desired_id} not found in channel_positions')

    # Sorting based on vertical position
    probe_ids_and_lfp_and_positions.sort(key=lambda x: x[2][0])

    # Extracting LFP data and vertical positions into separate arrays
    lfp_data_sorted = np.array([item[1] for item in probe_ids_and_lfp_and_positions])
    vertical_positions_sorted = np.array([item[2][0] for item in probe_ids_and_lfp_and_positions])

    # Calculating the difference between vertical positions
    vertical_diff = np.diff(vertical_positions_sorted)

    # Calculating the first and second derivatives of LFP data along the spatial dimension
    first_derivative = np.diff(lfp_data_sorted, axis=0) / vertical_diff[:, np.newaxis]
    second_derivative = np.diff(first_derivative, axis=0) / vertical_diff[:-1, np.newaxis]

    # Creating a DataFrame for CSD data
    csd_df = pd.DataFrame(second_derivative.T)

    # print the shape of the CSD DataFrame
    print(csd_df.shape)
...
if __name__ == '__main__':
    ...
    # read in channels.csv file with probe metadata specifically for electrode vertical positions
    channels_df = pd.read_csv('/Volumes/Extreme SSD/Allen SDK/channels.csv')
    ...
    for nwb_master_file in nwb_master_files:
        ...
        for nwb_file_path in nwb_files:
            ...
            # Determine indices of electrodes in desired areas
            desired_indices = [i for i, loc in enumerate(nwb.electrodes['location']) if loc in desired_areas]

            # determine electrode ids in desired areas
            desired_ids = [nwb.electrodes['id'][i] for i in desired_indices]
            ...
            for actual_visual_area in actual_visual_areas:
                ...
                # Calculate the CSD DataFrame
                csd_df = calculate_csd(lfp_df, channels_df, desired_ids, desired_indices)

                probes_csd_df = []
                # Check if the data is mostly zeros
                if not csd_df.astype(bool).sum().sum() > 10000:
            ...
            # Iterate over each probe's csd_df
            for csd_data in probes_csd_df:
                csd_df = csd_data['csd_data']
                probe_name = csd_data['probe_name']
                visual_area = csd_data['visual_area']
                sampling_rate = csd_data['sampling_rate']
                ...
                for iter_count in range(4):
                    ...
                    random_channels = random.sample(list(csd_df.columns), 3)
                    ...
                    for i, presentation in enumerate(presentations):
                        ...
                        for interval in intervals:
                            max_phi, max_phi_state = compute_phi_csd_rand_int(presentation, csd_df, interval,
