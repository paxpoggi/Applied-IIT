...
def scatter_plot_phi_values(all_iterations_phi_values, probe_name, visual_area):
    ...
    file_name = f'phi_values_rand_gamma_{visual_area} for {probe_name}'
    file_path = os.path.join(folder_name, file_name)
    plt.savefig(file_path, bbox_inches='tight')
    # Save the figure with proper padding
    plt.savefig(f'phi_values_rand_gamma_{visual_area} for {probe_name}.png', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    for nwb_master_file in nwb_master_files:
        ...
        for nwb_file_path in nwb_files:
            ...
            desired_areas = ['LGd', 'VISp', 'VISpm', 'VISl', 'VISrl', 'VISam', 'APN', 'CA1']
            ...
            for probe_dict in probes_lfp_df:
                ...
                folder_name = '/Users/poggi/Documents/Maier Lab/IIT Notebooks/phi_values_lfp_gamma_extra_brain_areas'
                file_name = f'lfp_gamma_phi_data_{probe_name}.csv'
                file_path = os.path.join(folder_name, file_name)

                # Flatten the data for CSV output
                flattened_data = [
                    {'probe_name': probe_name, 'visual_area': visual_area, 'iteration_phi_values': iteration_phi_values}
